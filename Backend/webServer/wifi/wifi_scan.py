...
class WiFi:
        ...
        def scan(self):
                ...
                for i in range(1,len(scan_split)):
                        dict_result = {}
                        mac = re.findall('Address: ([A-Z0-9]{2}:[A-Z0-9]{2}:[A-Z0-9]{2}:[A-Z0-9]{2}:[A-Z0-9]{2}:[A-Z0-9]{2})\s',scan_split[i])
                        ...
                        Encryption = re.findall('key:(.*?)\s',scan_split[i])
                        ...
                        Quality = re.findall('Quality=(.*?)\s',scan_split[i])
                        dict_result['Quality'] = Quality[0]
                        Signal = re.findall('Signal level=(.*?)[\/\s]',scan_split[i])
