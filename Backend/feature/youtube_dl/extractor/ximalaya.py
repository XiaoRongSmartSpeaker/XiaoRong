...
class XimalayaIE(XimalayaBaseIE):
    ...
    _TESTS = [{'url': 'http://www.ximalaya.com/61425525/sound/47740352/',
               'info_dict': {'id': '47740352',
                             'ext': 'm4a',
                             'uploader': '小彬彬爱听书',
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            url, audio_id, note='Download sound page for %s' %
            audio_id, errnote='Unable to get sound page')

        audio_info_file = '%s://m.ximalaya.com/tracks/%s.json' % (
            scheme, audio_id)
        audio_info = self._download_json(
            audio_info_file, audio_id, 'Downloading info json %s' %
            audio_info_file, 'Unable to download info file')
        ...
        if not audio_description:
            ...
            audio_description = self._download_webpage(
                audio_description_file, audio_id, note='Downloading description file %s' %
                audio_description_file, errnote='Unable to download descrip file', fatal=False)
        ...
        return {
            ...
            'uploader_url': self._USER_URL_FORMAT % (scheme,
                                                     audio_uploader_id) if audio_uploader_id else None,
...
class XimalayaAlbumIE(XimalayaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            self._TEMPLATE_URL %
            (scheme, uid, playlist_id), playlist_id, note='Download album page for %s' %
            playlist_id, errnote='Unable to get album info')
