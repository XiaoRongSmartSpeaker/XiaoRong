...
class ACastIE(ACastBaseIE):
    ...
    _VALID_URL = r'''(?x)
                    https?://
                        ...
                        (?P<channel>[^/]+)/(?P<id>[^/#?]+)
                    '''
    _TESTS = [{'url': 'https://www.acast.com/sparpodcast/2.raggarmordet-rosterurdetforflutna',
...
class ACastChannelIE(ACastBaseIE):
    ...
    def suitable(cls, url):
        return False if ACastIE.suitable(url) else super(
            ACastChannelIE, cls).suitable(url)
