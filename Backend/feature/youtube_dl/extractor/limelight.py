        ...
        try:
            return self._download_json(
                ...
                (self._PLAYLIST_SERVICE_PATH,
                 ...
                 method),
                item_id,
                'Downloading PlaylistService %s JSON' %
                method,
                fatal=fatal,
                headers=headers)
    ...
    def _extract_info(self, pc, mobile, i, referer):
        ...
        for stream in pc_item.get('streams', []):
            ...
            if not stream_url or stream.get(
                    'drmProtected') or stream_url in urls:
                continue
        ...
        for mobile_url in mobile_item.get('mobileUrls', []):
            ...
            if not media_url or format_id in (
                    'Widevine', 'SmoothStreaming') or media_url in urls:
                continue
