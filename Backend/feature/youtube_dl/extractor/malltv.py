...
class MallTVIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'https://www.mall.tv/18-miliard-pro-neziskovky-opravdu-jsou-sportovci-nebo-clovek-v-tisni-pijavice',
        ...
        return merge_dicts(
            {
                ...
                'duration': int_or_none(
                    video.get('DurationSeconds')) or parse_duration(
                    video.get('Duration')),
                ...
                'average_rating': float_or_none(
                        try_get(
                            video,
                            lambda x: x['EntityRating']['AvarageRate'])),
