...
class InaIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'http://www.ina.fr/video/I12055569/francois-hollande-je-crois-que-c-est-clair-video.html',
    ...
    def _real_extract(self, url):
        ...
        def media_ns_xpath(x): return self._xpath_ns(
            x, 'http://search.yahoo.com/mrss/')
        content = item.find(media_ns_xpath('content'))

        def get_furl(x): return xpath_attr(content, media_ns_xpath(x), 'url')
        formats = []
